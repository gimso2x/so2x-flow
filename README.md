...
> **Quick start**
> - 설치는 프로젝트 로컬에 한다.
> - 기본 성공 기준은 **step 1/4~4/4 출력 + 필수 파일 4개 확인**이다.
...
| 필수 파일 4개 | `.claude/skills/flow-init.md`, `.workflow/scripts/execute.py`, `.workflow/scripts/doctor.py`, `.workflow/config/ccs-map.yaml` 존재 |
...
3. `.claude/skills/flow-init.md`, `.workflow/scripts/execute.py`, `.workflow/scripts/doctor.py`, `.workflow/config/ccs-map.yaml` 존재 여부를 확인해. 필요하면 선택적으로 `python3 .workflow/scripts/doctor.py --brief` 도 실행해.
