...
Codex에서는 루트 `AGENTS.md`를 먼저 읽고 `python3 flow.py ...`만 기억하면 된다.
`flow.py`는 `.workflow/scripts/execute.py`와 `.workflow/scripts/doctor.py`를 감싼 얇은 래퍼라서 source of truth는 그대로 유지된다.

```bash
python3 flow.py doctor --brief
python3 flow.py init "개인용 운동 코칭 앱 MVP" --dry-run
python3 flow.py plan "로그인 기능 작업 분해" --dry-run
python3 flow.py feature "로그인 기능 구현" --dry-run
python3 flow.py status --json
```

Codex에게 바로 맡길 때는 아래처럼 시작하면 된다.

```text
AGENTS.md와 .workflow/docs/*를 먼저 읽고 현재 상태를 `python3 flow.py doctor --brief`로 확인해.
승인된 plan이 없으면 `python3 flow.py plan "로그인 기능 작업 분해" --dry-run`까지만 실행하고,
이미 승인된 plan이 있으면 그 plan을 기준으로 다음 액션을 제안해.
```

`feature --dry-run`은 surface 확인용으로 바로 돌릴 수 있지만, live feature 실행은 여전히 승인된 plan이 있어야 한다.
